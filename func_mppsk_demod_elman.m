function [snr,pe]= func_mppsk_demod_elman(M,N,K,code)      %(snr)   %,alpha)
...
M %=  %8 %4 % 8  %2  %4  %2  %16   %64    %128;     %128进制
K %=  %32 %2  % 57 %48   % 20  %2  %99  %198  %24    %6     %3; %2;    % every diverse is 2T
N  %=  %400  %10  %399 %400  %180  %10  %400;   %260;   % every base band signal is 260T
code
...
snr=[-6 -4 -2 0 2 4 6];
%snr=[-6 -5 -4 -3 -2 -1 0 1 2 3 4];
%snr=[-6 -3 0 3 6 9 12 15 18 21];
...
%Nblocks =[80 80 80 80 80 160 160 160 160 160 160 160 160 160];
[m n]=size(snr);
pe=[];
mppskfilter=[];
err=zeros(1,n);
        ...
        for num = 1:Nsymbols
           %  the whole diverse do not need to seperate 0 and 1 2 3.... 
            if( s_tx(num) == 0)       
                ...
                k = s_tx(num);                    
                head = f0(1:(k - 1)*K*(fs/fc));     %        f0(1:(k)*K*(fs/fc)); 
                middle=f1;
                tail = f0(1:(N - (k - rg)*K)*(fs/fc));    %  f0(1:(N - (k - rg+1)*K)*(fs/fc)); 
...
%         
    ...
    %    y_mppsk=mppsk+noise;    %+pulsenoise;    %   
         y_mppsk=mppsk+pulsenoise; 
       %  y_mppsk=awgn(mppsk,snr(i));
    % y_mppsk=mppsk;
     ...
     for mm=1:length (y_mppsk)
        ...
        y_mppsk_baoluo=y_mppskfilter;    %    %
   %     y_mppsk_baoluo=y_mppsk;
          %******************取绝对值******************************
...
%% ***********learn the signals' input and output under this SNR**************     
 % train_t=1:n1;
     if j==1 
      net=elmannet(1:2,code,'trainscg' );  %'trainlm');   %  %10  %'traingdx');  %100
      net.trainParam.epochs=5000;  % iterations
      net.trainParam.lr=0.001;  % learning ratio
      net.trainParam.goal=0.00004;   % 0.0000002;  % mu biao wu cha
      net.trainParam.max_fail=10;
    %  net.trainParam.showWindow=false;
      y_mppsk_baoluo_tmp=reshape(y_mppsk_baoluo,n1,Nsymbols);  %a column, a sample
    ...
    %  y_mppsk_baoluo_tmp=(y_mppsk_baoluo_tmp)';
     ...
     %   net=train(net,y_mppsk_baoluo_tmp,s_tx);
