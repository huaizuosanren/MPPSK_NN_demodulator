function [snr,pe]= func_mppsk_demod_diffVd(Vd,alpha,code)
%clc;
%clear all;
%close all;
%Vd=5;
Vd
alpha
code   % the code for the neural net
M=16  %8   %4 % 64 %  %2  %4  %2  %16   %64    %128;     %128进制
K =24  %32  %2 % 4  % 57  % % 20  %2  %99  %198  %24    %6     %3; %2;    % every diverse is 2T
N =400  %400 %10 %256   %399 %  %180  %10  %400;   %260;   % every base band signal is 260T
A=1;     % amplifier is 1
B=1;
rg=0;   
...
%Nblocks =[80 80 80 200 500 1000 5000 5000 5000 5000 5000 5000 5000 5000];
%Nblocks =[200 500 1000 3000 3000 3000];
Nblocks = [100 500 1000 1000 5000 5000 5000 10000 10000];
[m n]=size(snr);
pe=[];
mppskfilter=[];

Nsymbols = 1000;  %5000;  %20000;  %5000;
 ...
 %   a=[1,-3.6512241163814698 , 5.1727286626648894 ,-3.3577825365961242, 0.84572301542400019];
  ...
  %   a=[1,-4.5620074920961651,9.5862839416819483,-11.566980661101638,8.4523528839743243,-3.5467147693005732,0.6855154433139603];
   ...
   %  a_env=conv( conv( Hd.sosMatrix(1,4:6),Hd.sosMatrix(2,4:6) ) ,Hd.sosMatrix(3,4:6));
    ...
    Hd=tianxian;   %tianxian700M_70M_10M; % the tianxian filter
