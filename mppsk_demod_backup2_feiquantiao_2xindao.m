...
M= 2 %128;     %128进制
K =2  %3; %2;    % every diverse is 2T
N =10  %400;   %260;   % every base band signal is 260T
A=1;     % amplifier is 1
B=1;
rg=0;   
...
%         
        ...
        y_mppsk(1,:)=mppsk+noise;    
        y_mppsk(2,:)= mppsk+pulsenoise;
    
  
         [mppskfilter,zf1]=filter(b,a,mppsk,zf1);
        [y_mppskfilter(1,:),zf2]=filter(b,a,y_mppsk(1,:),zf2);
        [y_mppskfilter(2,:),zf2]=filter(b,a,y_mppsk(2,:),zf2);
...
% %         %*****************取包络***********************************
        ...
        [y_mppsk_baoluo,zf_env2]=filter(Num_low,1,y_mppskfilter(yi,:),zf_env2);
...
%                           err_four(num4)=s_rcv(ii);
 ...
 pe(1,i)=err(1)/(Nblocks(i)*Nsymbols)
   pe(2,i)=err(2)/(Nblocks(i)*Nsymbols)
    num0 
    num1
    num2
    num3
