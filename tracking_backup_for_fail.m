function [trackResults, channel]= tracking(data,s_tx, settings)
...
%% initialize the chongji filter----------------------------
 ...
 load num_low;  %initialize the low pass filer
     zf_env1 = zeros(max(length(Num_low),1)-1,1); % if it does need to get the picture of snr-err it should be taken into cycle
    zf_env2 = zeros(max(length(Num_low),1)-1,1);
    Nsymbols=length(s_tx);
%%---------------------------------------------------------------------------------
...
%% chongji filter to get the baseband signals--------------------------------------
            ...
            fs=fc*8;  
            if loopcnt>=400
               % [mppskfilter,zf1]=filter(b,a,rawSignal,zf1);  % without noise
                [y_mppskfilter,zf2]=filter(b,a,rawSignal,zf2);   % with noise
       % mppskfilter=abs(mppskfilter);  % get abs
        y_mppskfilter=abs(y_mppskfilter);
        mppsk_length=length(y_mppskfilter);  %mppsk_length=length(mppskfilter);
        ...
        % get every signals' numbers to define the yuzhi-------------------------------
         ...
         if j==1
           ...
           if (mod(loopCnt,20)==0)
            ln=loopCnt/20
                mppsk_baoluo0(:,ln)= mppsk_baoluo((((Nsymbols/5)+1)+ind0(ln)-2)*n1+1:(((Nsymbols/5)+1)+ind0(ln)-1)*n1)';                    %mppsk_baoluo((1001+ind0(l)-2)*n1+1:(1001+ind0(l)-1)*n1)';
                y_mppsk_baoluo0(:,ln)=y_mppsk_baoluo((((Nsymbols/5)+1)+ind0(ln)-2)*n1+1:(((Nsymbols/5)+1)+ind0(ln)-1)*n1)';                                             %y_mppsk_baoluo((1001+ind0(l)-2)*n1+1:(1001+ind0(l)-1)*n1)';
            %end
            for l=1:M-1
