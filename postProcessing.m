function [trackResults]= postProcessing(data,mppsk,s_tx,settings)
...
%% Acquisition ============================================================
   ...
   % if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
        ...
        %Generate the carrier frequency ÓÃµ¥ÔØ²¨ÐÅºÅÈ¡´úÔ­ÎÄ¼þÐÅºÅ -----------
         %  data_cut=zeros(1,11*samplesPerCode,'int8');
         %   data_cut= data(1:11*samplesPerCode);
        %--- Do the acquisition -------------------------------------------
       

        %disp ('   Acquiring satellites...');
        %acqResults =acquisition(data_cut, settings);
...
%% Track the signal =======================================================
    ...
    [trackResults, channel] = tracking(data,mppsk,s_tx, settings);
