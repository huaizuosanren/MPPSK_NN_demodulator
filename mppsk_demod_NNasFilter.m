...
B=1;
rg=0;   
thita=pi;  % the diverse angleis pi
...
Nsymbols = 100; %1000;  %2000; % 5000;  %20000;  %5000;
...
%% ***********learn the signals' input and output under this SNR**************     
 % train_t=1:n1;
     if j==1
      ...
      mppsk_baoluo_tmp=reshape(mppsk_baoluo,n1,Nsymbols);
      clear y_mppsk_baoluo mppsk_baoluo;
  %************train the BPNN as a noise filter*********************
  netf=cascadeforwardnet(10);
  netf.trainParam.epochs=100;
  netf.trainParam.goal=0.04;
  netf.trainParam.lr=0.01;
  [y_mppsk_baoluo_tmpn,y_mppsk_baoluo_tmps]=mapminmax(y_mppsk_baoluo_tmp);
  [mppsk_baoluo_tmpn,mppsk_baoluo_tmps]=mapminmax(mppsk_baoluo_tmp);
  netf=train(netf,y_mppsk_baoluo_tmpn,mppsk_baoluo_tmpn);
 ...
 %    y_mppsk_baoluo_tmp=(y_mppsk_baoluo_tmp)';
     % s_rcvtmp=zeros(Nsymbols,n1);
        %for t=1:Nsymbols
         % y_mppsk_baoluo_tmp(1,:)=(y_mppsk_baoluo((t-1)*n1+1:t*n1))';
         % y_mppsk_baoluo_tmp(2,:)=train_t';
    %**********use netf to filter the noise***********
    y_mppsk_baoluo_tmp=mapminmax('apply',y_mppsk_baoluo_tmp,y_mppsk_baoluo_tmps);
    y_mppsk_baoluo_tmp=netf(y_mppsk_baoluo_tmp);
    y_mppsk_baoluo_tmp=mapminmax('reverse',y_mppsk_baoluo_tmp,'mppsk_baoluo_tmps');
...
%                           err_four(num4)=s_rcv(ii);
   ...
   pe(i)=err/(Nblocks(i)*Nsymbols)
    ...
    num2
    num3
    clear net y_mppsk_baoluo y_mppsk_baoluo_tmp y_mppsk;
   end
