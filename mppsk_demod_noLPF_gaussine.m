...
K =2  %24  %57 %48   % 20  %2  %99  %198  %24    %6     %3; %2;    % every diverse is 2T
N = 10  %400 %399 %400  %180  %10  %400;   %260;   % every base band signal is 260T
A=1;     % amplifier is 1
B=1;
rg=0;   
...
%Nblocks = [20 20 40 40 40 60 60 60 80 80 80 80 160 160 160 160 160 160];
[m n]=size(snr);
pe=[];
mppskfilter=[];
...
%% ***********learn the signals' input and output under this SNR**************     
 % train_t=1:n1;
     if j==1  
      net=patternnet(10);
      net.trainParam.epochs=200;   %100;  % max iterations
      net.trainParam.lr= 0.001; %0.001;  % learning ratio
