...
K =45  %50 % 57 %48   % 20  %2  %99  %198  %24    %6     %3; %2;    % every diverse is 2T
N = 400  %399 % %180  %10  %400;   %260;   % every base band signal is 260T
A=1;     % amplifier is 1
B=1;
rg=0;   
...
%Nblocks = [20 20 40 40 40 60 60 60];
[m n]=size(snr);
pe=[];
mppskfilter=[];
 ...
 %% 得到调制信号并加噪
        for num = 1:Nsymbols
           ...
           % if( s_tx(num) == 0)       
            ...
            %else                                        
                k = s_tx(num);                    
                head =     f0(1:(k)*K*(fs/fc)); %      f0(1:(k - 1)*K*(fs/fc));   
                middle=f1;
                tail =  f0(1:(N - (k - rg+1)*K)*(fs/fc));  %  f0(1:(N - (k - rg)*K)*(fs/fc));  
