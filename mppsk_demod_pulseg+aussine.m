...
M=  64  %2 %2   %3 %2  %64    %128;     %128jizhi
K = 6  %8  %8  %133  %198  %6     %3; %2;    % every diverse is 2T
N = 400  %20   %400;   %260;   % every base band signal is 260T
A=1;     % amplifier is 1
B=1;
rg=0;   
...
%                           err_four(num4)=s_rcv(ii);
   ...
   pepulse(i)=err/(Nblocks(i)*Nsymbols)
    num0
    num1
    num2
    num3
  ...
  %plot(s_rcv);
  %title('receive')
 %% ************** process for another kind of noise********************************
   for(i=1:n)
       % i=1;
     %   snr(i)=6;   %-6    %6;
         fprintf('SNR = %2gdB\n',snr(i));
    ...
    zf_shape=zeros(max(length(b_shape),length(a_shape))-1,1);
     ...
     for j = 1:1+20
        if(mod(j-1,20)==19)
            fprintf('time for loop:%d accomplished£º%f \n',Nblocks(i),(j-1)/Nblocks(i));
        end
 ...
 %%**************get the modemed signals and add noise************** 
        for num = 1:Nsymbols
            if( s_tx(num) == 0)
                mppsk(num, :) = f0;                      
            else                                        
...
%                           err_four(num4)=s_rcv(ii);
   ...
   peguassine(i)=err/(Nblocks(i)*Nsymbols)
    num0
    num1
    num2
    num3
  ...
  semilogy(snr,pepulse);
grid on;
hold on;
semilogy(snr,peguassine,'r');
legend('pulse noise','gaussine noise');
