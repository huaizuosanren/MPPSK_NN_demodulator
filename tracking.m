function [trackResults, channel]= tracking(data,mppsk,s_tx, settings)
...
remblkloc=0;
%get the location for the 200ms iir process-----------
rem200msloc=0;
remtxloc=0;
...
%% Read next block of data ------------------------------------------------            
            ...
            %get the blksize for the 200ms iir process-----------------
            size200ms=2000*blksize;
...
%%%%             d_pll_x = d_pll_x + PDIcode * (0.5*d_pll_w + d_pll_a2 * d_pll_w0f * FLL_discriminator + d_pll_a3 * d_pll_w0p2 * carrError); 
   ...
   %% chongji filter to get the baseband signals-----------------------------
            ...
            if mod(loopCnt,2000)==0
                y_mppsk=data(rem200msloc+1:rem200msloc+size200ms);
                mppsk_send=mppsk(rem200msloc+1:rem200msloc+size200ms);
                rem200msloc=rem200msloc+size200ms;
                s_txjiequ=s_tx(remtxloc+1:remtxloc+(loopCnt/20));
                remtxloc=remtxloc+(loopCnt/20);
                s_rcv=mppskiir(carrFreq,mppsk_send,y_mppsk,s_txjiequ);
                 trackResults(channelNr).s_rcvsave(remtxloc+1:remtxloc+(loopCnt/20)) =  s_rcv;
            end
   %% chongji filter end-----------------------------------------------        
