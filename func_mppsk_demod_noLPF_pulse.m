function [snr pe]=func_mppsk_demod_noLPF(snr,M,K,N)
%clc;
%clear all;
%close all;

%M= 4 %16  %8  %2  %4  %2  %16   %64    %128;     %128进制
%K = 2 %57 %48   % 20  %2  %99  %198  %24    %6     %3; %2;    % every diverse is 2T
%N = 10 %34 %399 %400  %180  %10  %400;   %260;   % every base band signal is 260T
A=1;     % amplifier is 1
B=1;
rg=0;   
...
%Nblocks = [20 20 40 40 40 60 60 60 80 80 160 160 160 160 160 160 160];
Nblocks = [1000 10000 20000 50000 50000  50000 50000];
%Nblocks =[80 160 320 400 400 400];
[m n]=size(snr);
pe=[];
mppskfilter=[];
...
%% ***********learn the signals' input and output under this SNR**************     
 % train_t=1:n1;
     if j==1
      net=patternnet(10);
      net.trainParam.epochs=200;  % iterations
      net.trainParam.lr=0.001;   % 0.00001;   %0.000005;  %  %0.001; % learning ratio
      net.trainParam.goal=0.00004;  % mu biao wu cha
    %  net.trainParam.showWindow=false;
   % net.trainFcn='trainrp';  %'traingdm';     %'trainoss';
      y_mppsk_tmp=reshape(y_mppskfilter,n1,Nsymbols);
    ...
    %  y_mppsk_baoluo_tmp=(y_mppsk_baoluo_tmp)';
     ...
     s_tx_tmp=full(s_tx_tmp);
        net=train(net,y_mppsk_tmp,s_tx_tmp);
     % end
 %     view net
      else
      y_mppsk_baoluo_tmp=reshape(y_mppskfilter,n1,Nsymbols);
 %    y_mppsk_baoluo_tmp=(y_mppsk_baoluo_tmp)';
